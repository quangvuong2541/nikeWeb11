    ...
    const listProduct = data.slice(0, 5).map((item, key) => {
        ...
        return (
            <li key={key} className={classes.product}>
                <Link to={`detailPage/${item._id}`}> <img src={item.img} className={classes.productImage} /></Link>
            </li>
        )
    })
    ...
    for (let i = 0; i < 5; i++) {
        listScrolLazyLoad.push(
            <li className={classes.product} key={i}>
                <Skeleton >
                    <img src="https://static.nike.com/a/images/c_limit,w_318,f_auto/t_product_v1/b0cccd97-5dc7-4dba-b720-ab7863109ebf/exploration-series-basketball-t-shirt-8dkWTQ.jpg" className={classes.productImage} />
                </Skeleton>
            </li>
        )
